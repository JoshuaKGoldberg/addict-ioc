...
class DependencyInjectionContainer {
  ...
  _getInstance(registration, config) {
    ...
    let instance = this.instances[registration.settings.key][config];
  ...
  _getConfig(key, config) {
    ...
    if (!config) {
      // TODO: Validate on the fly config?
      // return this.registrations[key].settings.config;
  ...
  _resolveConfig(key, config) {
    ...
    if (configType === 'function') {
      ...
      resolvedConfig = globalConfiguration.get(config);

      if (!resolvedConfig) {
        throw new Error(`The nconf config for '${config}' is undefined.`);
      }
    }
