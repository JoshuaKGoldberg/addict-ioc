...
class DependencyInjectionContainer {
    ...
    registrationKeys.forEach((registrationKey) => {
      ...
      for (let dependencyIndex = 0; dependencyIndex < dependencies.length; dependencyIndex++) {
        ...
        if (!dependencyRegistration) {
          ...
          parentRegistrationHistory.push(registration);
          ...
          const circularBreakFound = this._historyHasCircularBreak(parentRegistrationHistory, dependencyRegistration);
  ...
  _historyHasCircularBreak(parentRegistrationHistory, dependencyRegistration) {
    ...
    return parentRegistrationHistory.some((parentRegistration) => {
      ...
      if (this.config.circularDependencyCanIncludeSingleton && this.parentRegistration.settings.isSingleton) {
        return true;
      }

      if (this.config.circularDependencyCanIncludeLazy && parentRegistration.settings.isLazy) {

        if (parentRegistration.settings.lazyKeys.length === 0 ||
          parentRegistration.settings.lazyKeys.indexOf(dependencyRegistration.settings.key) >= 0) {
