...
class DependencyInjectionContainer {
  ...
  _getNewInstance(registration, config, resolvedKeyHistory) {
    ...
    if (registration.settings.bindFunctions) {
      ...
      this._bindFunctionsToInstance(instance);
    }
  ...
  _bindFunctionsToInstance(instance) {
    const instanceKeys = Object.getOwnPropertyNames(instance.__proto__)
  ...
  validateDependencies(optionalKeyOrType) {
    ...
    this._validateDependencies(registrationKeys);
  }
  ...
  _validateDependencies(registrationKeys) {
    ...
    registrationKeys.forEach((registrationKey) => {
      ...
      Array.prototype.push.apply(allDependencies, this.registrations[registrationKey].settings.dependencies);
    });

    registrationKeys.forEach((registrationKey) => {

      const keyIndex = allDependencies.indexOf(registrationKey);

      if (keyIndex >= 0) {

        allDependencies.splice(keyIndex, 1);
      }
    });

    if (allDependencies.length > 0) {
      throw new Error(`The following dependencies are not registered: ${allDependencies.toString()}`);
    }
  }
}
