...
class DependencyInjectionContainer {
  ...
  _resolveDependencies(registration, resolvedKeyHistory) {
    ...
    dependencies.forEach((dependency) => {
      ...
      const dependencyInstance = this._resolve(dependencyKey, null, resolvedKeyHistory, isLazy);
    ...
    registrationKeys.forEach((registrationKey) => {
      ...
      for (let dependencyIndex = 0; dependencyIndex < dependencies.length; dependencyIndex++) {
        ...
        if (!dependencyRegistration) {
          ...
          errors.push(`Dependency '${dependencyKeyOverwritten}' registered on '${registration.settings.key}' is missing.`);
          ...
          } else if (parentKeyHistory.indexOf(dependencyKeyOverwritten) >= 0) {

            errors.push(`Circular dependency on key '${dependencyKeyOverwritten}' detected.`);
            return;
          }
          ...
          this._validateOverwrittenKeys(registration);
  ...
  _validateOverwrittenKeys(registration) {
    ...
    overwrittenKeys.forEach((overwrittenKey) => {
      ...
      this._validateOverwrittenKey(registration, overwrittenKey);
    });
  }
