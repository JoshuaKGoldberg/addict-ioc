...
class DependencyInjectionContainer {
  ...
  _resolveDependencies(registration, resolvedKeyHistory) {
    ...
    dependencies.forEach((dependency) => {
      ...
      const dependencyInstance = this._resolve(dependency, null, resolvedKeyHistory, isLazy);
  ...
  _isDependencyLazy(registration, dependency) {
    ...
    const isLazy = registration.settings.isLazy
    && (registration.settings.lazyKeys.length === 0 || registration.settings.lazyKeys.indexOf(dependency) >= 0);
  ...
  _createInstance(registration, dependencies) {
    ...
    //
    //   const relativeRequirePath = path.join(this.config.requireRelativePath, registration.settings.type);
    //
    //   const requiredModule = require(relativeRequirePath);
    //
    //   if (typeof requiredModule !== 'function') {
    //
    //     instance = requiredModule;
    //
    //   } else {
    //
    //     instance = new requiredModule();
    //   }
    //
    // } else
    ...
    if (typeof type !== 'function') {
      ...
      if (registration.settings.isFactory) {
        ...
        instance = this._createInstanceByFactoryWithInjection(type, dependencies);

      } else {

        instance = this._createInstanceByConstructorWithInjection(type, dependencies);
      }
  ...
  _createInstanceByFactoryWithInjection(type, dependencies) {
    const instance = type.apply(undefined, dependencies);
    return instance;
  }
  ...
  _createInstanceByConstructorWithInjection(type, dependencies) {
    const instance = new(Function.prototype.bind.apply(type, [null].concat(dependencies)))();
    return instance;
  }
  ...
  _validateDependencies(registrationKeys, parentKeyHistory) {
    ...
    registrationKeys.forEach((registrationKey) => {
      ...
      for (let dependencyIndex = 0; dependencyIndex < dependencies.length; dependencyIndex++) {
        ...
        const dependencyKey = dependencies[dependencyIndex];

        const dependencyRegistration = this.registrations[dependencyKey];

        if (!dependencyRegistration) {

          errors.push(`Dependency '${dependencyKey}' registered on '${registration.settings.key}' is missing.`);
          ...
          } else if (parentKeyHistory.indexOf(dependencyKey) >= 0) {

            errors.push(`Circular dependency on key '${dependencyKey}' detected.`);
            return;
          }
          ...
          const deepErrors = this._validateDependencies([dependencyRegistration.settings.key], parentKeyHistory);

          if (deepErrors.length > 0) {

            errors.push(`Inner dependency errors for dependency '${dependencyKey}':
                ${deepErrors.toString()}`);
          }
        }
      }
