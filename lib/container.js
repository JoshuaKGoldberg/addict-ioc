...
class DependencyInjectionContainer {
  ...
  _getKeysForInstanceConfigurationsByKeyOrType(keyOrType) {
    return Object.keys(this.instances[keyOrType]);
  }
  ...
  _callSubscribers(registration, subscriptionKey, params) {
    ...
    const subscriberRegistrations = this._getSubscriberRegistrations(registration.key, subscriptionKey);

    subscriberRegistrations.forEach((subscriberRegistration) => {

      const subscribedInstances = this._getAllInstances[subscriberRegistration.key];

      subscribedInstances.forEach((subscribedInstance) => {

        const subscribedMethod = subscribedInstance[subscriberRegistration.method];
