...
class DependencyInjectionContainer {
  ...
  unregister(keyOrType) {
    const registration = this._getRegistration(keyOrType);
  ...
  _getRegistration(key) {
    ...
    if (!registration) {
      throw new Error(`The key '${key}' has not been registered.`);
    }
  ...
  _resolve(key, config, resolvedKeyHistory, isLazy) {
    ...
    let registration = this._getRegistration(key);

    if (!registration && typeof key !== 'string') {

      if (this._config.registrationDefaults.autoCreateMissingRegistrations) {

        registration = this._registerType(key);
