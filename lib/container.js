...
class DependencyInjectionContainer {
  ...
  setConfigProvider(getConfigCallback) {
  ...
  _getAllInstances(key, config, injectionArgs) {
    ...
    if (config) {
      ...
      configKeys = this._getKeysForInstanceConfigurationsByKey(key);
    }
  ...
  _getFunctionsToBind(registration, instance) {

    if (registration.settings.functionsToBind.length > 0) {

      return registration.settings.functionsToBind;
  ...
  _bindFunctionsToInstance(registration, instance) {
    ...
    const functionKeys = this._getFunctionsToBind(registration, instance);

    functionKeys.forEach((functionKey) => {

      this._bindFunctionToInstance(instance, functionKey);
    });
  }


  _bindFunctionToInstance(instance, functionKey) {

    if (functionKey === 'constructor') {
      return;
    }

    const keyType = typeof instance[functionKey];

    if (keyType === 'function') {

      const unboundKey = instance[functionKey];

      instance[functionKey] = unboundKey.bind(instance);
    }
  }
  ...
  _parseDependencies(registration) {
  ...
  _resolveDependencies(registration, resolvedKeyHistory) {
    ...
    const dependencyKeys = this._parseDependencies(registration);
    ...
    dependencyKeys.forEach((dependencyKey) => {
      ...
      const dependencyInstance = this._resolveDependency(registration, dependencyKey, resolvedKeyHistory);
  ...
  _resolveDependency(registration, dependency, resolvedKeyHistory) {
  ...
  _getTypeByRequire(registration) {
  ...
  _createInstance(registration, dependencies, injectionArgs) {
    ...
    if (registration.settings.isRequire) {
      ...
      type = this._getTypeByRequire(registration);
    }
  ...
  _getConfig(key, config) {
    ...
    const registrationConfig = this.registrations[key].settings.config;

    const resolvedRegistrationConfig = this._resolveConfig(key, registrationConfig);
    const resolvedConfig = this._resolveConfig(key, config);

    if (!resolvedRegistrationConfig && !resolvedConfig) {
      return undefined;
    }

    const finalConfig = Object.assign(resolvedRegistrationConfig, resolvedConfig);
  ...
  _resolveConfig(key, config) {
    ...
    if (configType === 'function') {
      ...
      resolvedConfig = this._resolveConfigFromFunction(key, config);

    } else if (configType === 'object') {

      resolvedConfig = this._resolveConfigFromObject(key, config);

    } else if (configType === 'string') {

      resolvedConfig = this._resolveConfigFromString(key, config);
  ...
  _resolveConfigFromFunction(key, config) {

    const resolvedConfig = config(key);
  ...
  _resolveConfigFromObject(key, config) {
  ...
  _resolveConfigFromString(key, config) {
    ...
    const resolvedConfig = this.externalConfigProvider(config, registration);
  ...
  validateDependencies(optionalKey) {
    ...
    registrationKeys.forEach((registrationKey) => {
      ...
      for (let dependencyIndex = 0; dependencyIndex < dependencies.length; dependencyIndex++) {
        ...
        if (!originalDependencyKeyRegistration) {
          ...
          errors.push(`Registration for '${originalDependencyKey}' overwritten with '${dependencyKeyOverwritten}' declared on registration for key '${registration.settings.key}' is missing.`);
        }
  ...
  getKeysByTags(tagOrTags) {
    ...
    const allTags = this._parseTags(tagOrTags);
    const foundKeys = [];
